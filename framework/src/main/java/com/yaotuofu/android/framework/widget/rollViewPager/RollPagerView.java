package com.yaotuofu.android.framework.widget.rollViewPager;
...
public class RollPagerView extends RelativeLayout implements OnPageChangeListener {
                ...
                if (isShown() && System.currentTimeMillis() - mRecentTouchTime > delay)
                    new Handler(Looper.getMainLooper()).post(new Runnable() {
                        ...
                        public void run() {
                            ...
                            if (mHintView != null)
                                ((HintView) mHintView).setCurrent(cur % realCount);
                        }
                    });
            }
        }, delay, delay);
    ...
    private void loadHintView() {
        ...
        ((HintView) mHintView).initView(mAdapter == null ? 0 : realCount, gravity);
    }
...
//        mViewPager.addOnPageChangeListener(this);
    ...
    private void dataSetChanged() {
        startPlay();
        if (mHintView != null)
            ((HintView) mHintView).initView(realCount, gravity);
    }
    ...
    public void onPageSelected(int arg0) {
        if (mHintView != null) ((HintView) mHintView).setCurrent(arg0 % realCount);
    }
