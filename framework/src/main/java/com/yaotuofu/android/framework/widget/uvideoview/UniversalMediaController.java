...
package com.yaotuofu.android.framework.widget.uvideoview;
...
/**
 * 来源 https://github.com/linsea/UniversalVideoView
 */
...
//    private boolean mFullscreenEnabled = false;
    ...
    private static final int sDefaultTimeout = 3600000;
    ...
    public void show(int timeout) {//只负责上下两条bar的显示,不负责中央loading,error,playBtn的显示.
        ...
        if (mTitleLayout.getVisibility() != VISIBLE) {
            mTitleLayout.setVisibility(INVISIBLE); // 暂时不需要显示标题
        }
        if (mControlLayout.getVisibility() != VISIBLE) {
            mControlLayout.setVisibility(INVISIBLE); // 暂时不需要显示控制栏
        }
        ...
        if (mProgress != null) {
            if (duration > 0) {
                ...
                if (null != mOnProgressListener) {
                    mOnProgressListener.onProgress((int) pos / 10);
                }
            }
